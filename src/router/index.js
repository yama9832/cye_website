...
const routes = [
  ...
  },
  {
    ...
    meta: {
      // タイトルなどはコンポーネント内で動的に設定することを推奨しますが、
      // ここではパンくずリストのみ定義します。
      breadcrumb: [{ text: 'ブログ', to: '/blog' }, { text: '記事' }], // 記事タイトルは動的に設定
      // keywordsやdescriptionも動的に設定するのが望ましい
    }
  },
  /* 参加・お問い合わせ */
  {
  ...
  if (to.meta.breadcrumb && to.meta.breadcrumb.length > 0) {
    const breadcrumbItems = [
      ...
      ...to.meta.breadcrumb.map((crumb, index) => {
        ...
        // 最後のパンくず要素には`item`プロパティを追加しない
        if (index < to.meta.breadcrumb.length - 1 || crumb.to) {
